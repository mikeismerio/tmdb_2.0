...
def app():
    ...
    with col2:
        year = str(movie.first_air_date)[:4] if movie.first_air_date else "N/A"
        ...
        st.markdown(f"**Popularidad:** {movie.popularity if movie.popularity else 'N/A'}")
        ...
        st.markdown(f"**Creador(es):** {movie.created_by if movie.created_by else 'No disponible'}")
